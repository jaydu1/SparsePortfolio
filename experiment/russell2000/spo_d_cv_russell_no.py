...
def fit(i, df, func, a):
    ...
    ws, _, _, _, _ = spo_nw_min(X, func, a, lambdas, None, screen=True, max_iter=int(1e5), f=200, tol=1e-8, nw_min=nw_min)
    id_lam = np.where(np.sum(ws>0., axis=0) >= nw_min)[0][0]
    time_elapsed = time.time() - t1

    if np.sum(ws[:, 0]>0.)>=nw_min:
        w = ws[:, id_lam]    
...
with Parallel(n_jobs=-1, verbose=100) as parallel:
    for func in range(2):
        ...
        for a in a_list:
            ...
            np.savez('result/res_russell_%d_%s_%.2f_no.npz'%(nw_min, func_names[func],a), 
